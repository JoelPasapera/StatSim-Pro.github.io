...
StatSim Pro is a single-page, **zero-build, dependency-free** browser app (Spanish UI) for psychology/social-science students. It does two things: simulates statistical datasets from target means/SDs, and runs an SPSS-style analyzer. The analyzer offers two analysis types (selected by the `tipoAnalisis` radio): **correlation** between two quantitative variables (normality test → Pearson/Spearman → CI/effect size/regression → hypothesis test → auto-generated thesis prose) and **two-group comparison** of a quantitative variable across a binary grouping variable (normality per group + Levene → Student/Welch t-test or Mann-Whitney U → Cohen's d). Everything runs client-side; no server, no package manager, no transpilation.
