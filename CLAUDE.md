...
StatSim Pro is a website that aims to be as ambitious as possible, **zero-build, dependency-free** browser app (Spanish UI) for psychology/social-science students. For now, it does these two things: simulates statistical datasets from target means/SDs, and runs an SPSS-style analyzer. The analyzer offers three analysis types (selected by the `tipoAnalisis` radio): **correlation** between two quantitative variables (normality test → Pearson/Spearman → CI/effect size/regression → hypothesis test → auto-generated thesis prose) and **group comparison** of a quantitative variable across a grouping variable: 2 groups → Student/Welch t-test or Mann-Whitney U (with Cohen's d); 3+ groups → one-way ANOVA or Kruskal-Wallis (with η²/ε² and Bonferroni post-hoc), chosen by normality per group + Levene; and **association** between two categorical variables (chi-square test of independence with Cramér's V on a contingency table). Everything runs client-side; no server, no package manager, no transpilation.
